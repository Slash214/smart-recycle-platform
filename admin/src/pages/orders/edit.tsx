...
import { Form, Input, Select, InputNumber, Button, Space, Card, Row, Col, Typography, Divider } from "antd";
...
export const OrderEdit = () => {
  ...
  React.useEffect(() => {
    if (query?.data?.data) {
      const r = query.data.data as Record<string, unknown>;
      const rawDevices = r.devices as Array<{ model?: string; memory?: string; unit?: string; qty?: number }> | undefined;
      const devices =
        Array.isArray(rawDevices) && rawDevices.length > 0
          ...
          : [{ model: "", memory: "", unit: "whole" as const, qty: 1 }];
      formProps.form?.setFieldsValue({
        ...r,
        remark_images: Array.isArray(r.remark_images) ? r.remark_images : [],
        devices,
      });
    }
  ...
  return (
    <Edit saveButtonProps={saveButtonProps}>
      ...
      >
          ...
          <Row gutter={12}>
            ...
            <Col span={12}>
              <Form.Item
                ...
                extra="填写下方回收明细后，将自动按明细数量汇总；无明细时以本字段为准"
              >
        ...
        <Card size="small" title="回收明细" style={{ marginBottom: 16 }}>
          <Text type="secondary">机型 / 内存 / 整机或单板 / 数量</Text>
          <Divider style={{ margin: "12px 0" }} />
          <Form.Item label="回收明细（机型 / 内存 / 整机·单板 / 数量）">
                ...
                {fields.map(({ key, name, ...restField }) => (
                  <Card key={key} size="small" style={{ marginBottom: 10 }}>
                    <Row gutter={10} align="middle">
                      <Col span={7}>
                        <Form.Item {...restField} name={[name, "model"]} label="机型" rules={[{ required: true, message: "机型" }]}>
                          <Input placeholder="机型" />
                        </Form.Item>
                      </Col>
                      <Col span={6}>
                        <Form.Item {...restField} name={[name, "memory"]} label="内存" rules={[{ required: true, message: "内存" }]}>
                          <Input placeholder="内存" />
                        </Form.Item>
                      </Col>
                      <Col span={5}>
                        <Form.Item {...restField} name={[name, "unit"]} label="类型" rules={[{ required: true }]}>
                          <Select
                            placeholder="类型"
                            options={[
        ...
        <Card size="small" title="物流与状态" style={{ marginBottom: 16 }}>
          <Row gutter={12}>
            <Col span={12}>
              <Form.Item
                label={"报价金额"}
          ...
          <Row gutter={12}>
            ...
            <Col span={12}>
              <Form.Item label={"快递公司"} name={["express_company"]}>
                ...
                <>
                  {fields.map(({ key, name, ...restField }) => (
                    <Space key={key} style={{ display: "flex", marginBottom: 8 }} align="baseline">
                      <Form.Item {...restField} name={[name]} rules={[{ required: true, message: "请输入图片URL" }]}>
